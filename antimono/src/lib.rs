...
/// init a node's kernel state
fn  init_sys_state() -> bool {
    // convert from physical addresses
    // convert from physical addresses to userland vptrs
    // create the root cnode
...
fn init_cpu(mask_legacy_irqs: bool) -> bool {
    // init_vm
    // init cpu's descriptor table registers (GDTR ,IDTR ,LDTR,TR)
    // init_sysenter_msrs or init_syscall_msrs
