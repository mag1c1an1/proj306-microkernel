...
impl Elf {
    pub fn parse_elf(input: &[u8]) -> EmberResult<Self> {
        ...
        for index in 0..ph_count {
            ...
            let ph64 = match program_header {
                ...
                xmas_elf::program::ProgramHeader::Ph32(_) => {
                    return Err(EmberError::Other("Not 64 byte executable"));
                }
            };
...
impl ElfHeader {
    fn parse_elf_header(header: Header) -> EmberResult<Self> {
        let pt1 = *header.pt1;
        let pt2 = match header.pt2 {
            ...
            _ => return Err(EmberError::Other("parse elf failed"))
        };
...
fn check_elf_header(elf_header: &ElfHeader) -> EmberResult<()> {
    // 64bit
    ...
    if elf_header.pt1.data() != header::Data::LittleEndian {
        return Err(EmberError::Other("Not little endian executable"));
    }
    // system V ABI
    ...
    if elf_header.pt2.machine.as_machine() != header::Machine::X86_64 {
        return Err(EmberError::Other("Not x86_64 executable"));
    }
    ...
    if elf_type != header::Type::Executable && elf_type != header::Type::SharedObject {
        return Err(EmberError::Other("Not executable file"));
    }
    Ok(())
}
