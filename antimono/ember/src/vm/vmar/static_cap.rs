...
use anti_frame::vm::{Vaddr, VmIo};
use anti_rights::{Dup, Rights, TRightSet, TRights};
...
impl<R: TRights> Vmar<TRightSet<R>> {
    ...
    ///
    /// ```
    /// use anti_std::prelude::*;
    /// use anti_std::vm::{PAGE_SIZE, Vmar, VmoOptions};
    ///
...
impl<R: TRights> VmarRightsOp for Vmar<TRightSet<R>> {
    ...
    fn check_rights(&self, rights: anti_rights::Rights) -> Result<()> {
        todo!()
    }
}
