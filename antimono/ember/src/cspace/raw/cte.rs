...
impl Default for CTE {
    fn default() -> Self {
        Self {
            raw_cap: RawCap::new_null_cap(),
            mdb_node: MdbNode::default(),
        }
    }
}
...
//         self as *const CTE as usize
//     }
//
//     pub fn get_offset_slot(&mut self, index: usize) -> &'static mut Self {
//         convert_to_mut_type_ref::<Self>(self.get_ptr() + core::mem::size_of::<CTE>() * index)
//     }
//
