use crate::{error::Errno, vm::vmar::Vmar, Result};
use alloc::{ffi::CString, vec::Vec};
use anti_frame::vm::Vaddr;
use anti_frame::vm::VmIo;
use anti_rights::Full;
use pod::Pod;

use crate::{return_errno_with_message, BIT};
...
pub fn read_cstring_from_vmar(vmar: &Vmar<Full>, addr: Vaddr, max_len: usize) -> Result<CString> {
    ...
    read_one_byte_at_a_time_while!(
        cur_addr % core::mem::size_of::<usize>() != 0 && buffer.len() < max_len
    );
    ...
    while (buffer.len() + core::mem::size_of::<usize>()) <= max_len {
        ...
        cur_addr += core::mem::size_of::<usize>();
    }
