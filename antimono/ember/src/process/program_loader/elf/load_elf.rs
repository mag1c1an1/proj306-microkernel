...
use anti_frame::{
    task::Task,
    vm::{Vaddr, VmIo, PAGE_SIZE},
};
use anti_rights::{Full, Rights};
...
pub fn load_elf_to_vm(
    ...
    //     Some(lookup_and_parse_ldso(&parsed_elf, elf_binary)?)
    // } else {
    //     None
    // };
    // let ldso = None;

    match init_and_map_vmos(process_vm, &parsed_elf, elf_binary) {
        Ok((entry_point, mut aux_vec)) => {
            ...
            init_stack_writer.map(|writer| writer.write().unwrap());

            let user_stack_top = process_vm
                .init_stack_reader()
                .map(|reader| reader.user_stack_top());
            Ok(ElfLoadInfo {
                entry_point,
                user_stack_top,
            })
        ...
        Err(_) => {
            ...
            trace!("load elf to vm failed");
            Task::current().exit();
        }
    }
}

// fn lookup_and_parse_ldso(elf: &Elf, elf_binary: &[u8]) -> Result<(Arc<Dentry>, Elf)> {
//     let ldso_file = {
...
fn init_and_map_vmos(
    ...
    //     None
    // };
    // let ldso_load_info = None;

    let elf_map_addr = map_segment_vmos(parsed_elf, root_vmar, elf_binary)?;
    ...
    // only supported statically linked executable
    let entry_point = parsed_elf.entry_point();
    ...
    //     // ldso itself
    //     parsed_elf.entry_poibent() + elf_map_addr
    // } else {
...
pub struct ElfLoadInfo {
    entry_point: Vaddr,
    user_stack_top: Option<Vaddr>,
}

impl ElfLoadInfo {
    pub fn new(entry_point: Vaddr, user_stack_top: Option<Vaddr>) -> Self {
        Self {
            entry_point,
            user_stack_top,
        }
    ...
    pub fn user_stack_top(&self) -> Option<Vaddr> {
        self.user_stack_top
    }
}

/// init vmo for each segment and then map segment to root vmar
pub fn map_segment_vmos(elf: &Elf, root_vmar: &Vmar<Full>, elf_binary: &[u8]) -> Result<Vaddr> {
    ...
    assert!(
        !elf.is_shared_object(),
        "now only support statically linked executable"
    );
    ...
    for program_header in &elf.program_headers {
        ...
        if type_ == program::Type::Load {
            ...
            let (vmo, anonymous_map_size) = init_segment_vmo(program_header, elf_binary)?;
            map_segment_vmo(
                program_header,
                vmo,
                anonymous_map_size,
...
fn init_segment_vmo(program_header: &ProgramHeader64, elf_binary: &[u8]) -> Result<(Vmo, usize)> {
    trace!(
    ...
    let segment_vmo = {
        ...
        VmoOptions::<Full>::new(vmo_size).alloc()?
    };
    ...
    segment_vmo.write_slice(0, slice)?;
